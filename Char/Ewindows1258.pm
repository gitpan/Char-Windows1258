...
if ($^O =~ /\A (?: MSWin32 | NetWare | symbian | dos ) \z/oxms) {
    if ($ENV{'ComSpec'} =~ / (?: COMMAND\.COM | CMD\.EXE ) \z /oxmsi) {
        ...
        for (@ARGV) {
            ...
            elsif (m/\A (?:$q_char)*? [*?] /oxms and (my @glob = Char::Ewindows1258::glob($_))) {
                push @argv, @glob;
            }
            else {
                push @argv, $_;
    ...
    for my $string (@string) {
        ...
        for (my $i=0; $i <= $#char; $i++) {
            ...
            # /i modifier
                ...
                my $lc = Char::Ewindows1258::lc($char[$i]);
                if ($uc ne $lc) {
                    $char[$i] = '[' . $uc . $lc . ']';
                }
            }
...
sub _octets {
    ...
    if ($length == 1) {
        ...
        if (((caller(1))[3] ne 'Char::Ewindows1258::_octets') and ($modifier =~ m/i/oxms)) {
            if ($a == $z) {
...
#               $pattern .= "(?:$your_char)",   # UNIX style
            ...
            elsif ((my $uc = Char::Ewindows1258::uc($char)) ne $char) {
                $pattern .= $uc;
            }
            else {
        ...
        my $matchsub = sub { Char::Ewindows1258::uc($_[0]) =~ m{\A $pattern \z}xms };
...
=item Split string
  ...
  $string on every match. If the pattern doesn't match at all, Char::Ewindows1258::split returns
...
=item Chop string
  ...
  The Char::Ewindows1258::chop operator is used primarily to remove the newline from the end of
  an input record but is more efficient than s/\n$//. If no argument is given, the
  function chops the $_ variable.

=item Index string

  $pos = Char::Ewindows1258::index($string,$substr,$position);
  $pos = Char::Ewindows1258::index($string,$substr);
  ...
  the Char::Ewindows1258::index function returns -1.

=item Reverse index string

  $pos = Char::Ewindows1258::rindex($string,$substr,$position);
  $pos = Char::Ewindows1258::rindex($string,$substr);

  Works just like Char::Ewindows1258::index except that it returns the position of the last
...
=item Make character
  ...
  set. For example, Char::Ewindows1258::chr(65) is "A" in either ASCII or Windows-1258, not Unicode,
  and Char::Ewindows1258::chr(0x82a0) is a Windows-1258 HIRAGANA LETTER A. For the reverse of
  Char::Ewindows1258::chr, use Char::Windows1258::ord.
