...
#
# Windows-1258 fold case lower case without parameter
#
sub Char::Ewindows1258::fc_() {
    my $s = $_;
    return join '', map {defined($fc{$_}) ? $fc{$_} : $_} ($s =~ m/\G ($q_char) /oxmsg);
...
#               $pattern .= "(?:$your_char)",   # UNIX style
        ...
        my $matchsub = sub { Char::Ewindows1258::fc($_[0]) =~ m{\A $pattern \z}xms };
...
=item Chop string
  ...
  character chopped. The Char::Ewindows1258::chop function is used primary to remove the newline
...
=item Reverse index string
  ...
  This function works just like Char::Ewindows1258::index except that it returns the byte
...
=item Lower case string
  ...
  This function returns a lowercased version of Windows-1258 $string (or $_, if
  $string is omitted). This is the internal function implementing the \L escape
  in double-quoted strings.

  You can use the Char::Ewindows1258::fc function for case-insensitive comparisons via Char::Windows1258
  software.
...
=item Lower case first character of string
  ...
  This function returns a version of Windows-1258 $string with the first character
...
=item Upper case string
  ...
  This function returns an uppercased version of Windows-1258 $string (or $_, if
...
=item Fold case string
  ...
  New to Char::Windows1258 software, this function returns the full Unicode-like casefold of
  Windows-1258 $string (or $_, if omitted). This is the internal function implementing
...
=item Filename expansion (globbing)
  ...
  The Char::Ewindows1258::glob function grandfathers the use of whitespace to separate multiple
